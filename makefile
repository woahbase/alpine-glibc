...
GLIBCVERSION ?= $(call get_glibc_version)
...
# {{{ -- flags
# buildtime flags
# use custom APKBUILD if available
APKBUILD ?= $(if $(wildcard files/APKBUILD.$(ARCH)),APKBUILD.$(ARCH),APKBUILD)
...
LABELFLAGS ?= \
	...
	--label online.woahbase.glibc-version=$(GLIBCVERSION) \
	...
	--label org.opencontainers.image.version=$(GLIBCVERSION) \
...
BUILDFLAGS ?= \
	$(CACHEFLAGS) \
	$(LABELFLAGS) \
	--build-arg APKBUILD=$(APKBUILD) \
	--build-arg GLIBCARCH=$(call get_src_arch) \
	--build-arg GLIBCVERSION=$(if $(GLIBCVERSION),$(GLIBCVERSION),$(error GLIBCVERSION is not defined)) \
	...
	--rm \
	--tag $(IMAGETAG) \
	# --build-arg COMPILERSRC=ubuntu:latest \
	#
...
push : SKIP_VERSIONTAG ?= $(if $(GLIBCVERSION),,1)
push : VERSIONTAG ?= $(subst $(ARCH),$(ARCH)$(if $(GLIBCVERSION),_$(GLIBCVERSION),),$(IMAGETAG))
push : BUILDDATETAG ?= $(subst $(ARCH),$(ARCH)$(if $(GLIBCVERSION),_$(GLIBCVERSION),)_$(BUILDDATE),$(IMAGETAG))
push : ## push image
	if [ -z "$(SKIP_$(ARCH))" ]; \
	then \
		docker push $(IMAGETAG); \
		if [ -z "$(SKIP_VERSIONTAG)" ] && [ -n "$(GLIBCVERSION)" ];\
		then \
...
# push image with version tag (if available)
# to skip pushing version set SKIP_VERSIONTAG to non empty string e.g. 1, default is unset if version given
push_registry_% : SKIP_VERSIONTAG ?= $(if $(GLIBCVERSION),,1)
push_registry_% : REGDSTTAG ?= $(subst $(REGISTRY),$(subst push_registry_,,$@),$(IMAGETAG))
push_registry_% : REGDSTVERSIONTAG ?= $(subst $(ARCH),$(ARCH)_$(GLIBCVERSION),$(REGDSTTAG))
push_registry_% : REGDSTBUILDDATETAG ?= $(subst $(ARCH),$(ARCH)$(if $(GLIBCVERSION),_$(GLIBCVERSION),)_$(BUILDDATE),$(REGDSTTAG))
...
annotate_version : TAGNAME ?= $(if $(GLIBCVERSION),$(GLIBCVERSION),$(error GLIBCVERSION is not defined))
annotate_version : manifest annotate ## annotate and push `VERSION` tag

annotate_date : SKIP_ANNOTATE ?= $(if $(SKIP_BUILDDATETAG),1,)
annotate_date : TAGNAME ?= $(if $(GLIBCVERSION),$(GLIBCVERSION)_,$(error GLIBCVERSION is not defined))$(BUILDDATE)
...
regbinfmt : ## register binfmt for multiarch on x86_64
	if [ "$(ARCH)" != "$(HOSTARCH)" ]; then \
		docker run --rm --privileged $(QEMUIMAGE) --reset -p yes  --credential yes; \
	fi;
	# credential=yes needed to run sudo inside container at build time, else effective UID is non-zero
...
# gets latest release version from https://ftp.gnu.org/gnu/glibc/
# $1 = architecture (format: armhf for armv6l, armv7 for armv7l, x86_64 and aarch64 are used as-is.)
# and dont refetch if file newer than 8 hours
define get_glibc_version
$(shell \
	if [ ! -f $(CURDIR)/GLIBCVERSION ] \
	|| [ "$$(( ($$(date +%s) - $$(stat $(CURDIR)/GLIBCVERSION  -c %Y)) / 3600 ))" -gt 8 ]; \
	then \
		curl -jsSL \
		https://ftp.gnu.org/gnu/glibc/ \
		2>/dev/null \
		| grep -Eo   '>glibc-([0-9\.]+).tar.gz' \
		| sed  -Ee 's!>glibc-([0-9\.]+).tar.gz!\1!' \
		| sort -uV \
		| tail -n 1 > $(CURDIR)/GLIBCVERSION;
	fi;
	cat $(CURDIR)/GLIBCVERSION;)
endef
# grep -oP "(?<=glibc-)[0-9\.]+(?=.tar.gz)" does not work in alpine (no -P)
# replaced by grep + sed

# maps apk build platform to ARCH tags
SRC_ARCH_MAP := \
	'aarch64') echo -n 'aarch64';; \
	'armhf'  ) echo -n 'armhf'  ;; \
	'armv7l' ) echo -n 'armv7'  ;; \
