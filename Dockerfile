...
RUN mkdir -p ${ABUILD_KEY_DIR} \
    && openssl genrsa -out ${ABUILD_KEY_DIR}/${PACKAGER}-key.pem 2048 \
    && openssl rsa -in ${ABUILD_KEY_DIR}/${PACKAGER}-key.pem -pubout -out /etc/apk/keys/${PACKAGER}.rsa.pub \
    && echo "PACKAGER_PRIVKEY=\"${ABUILD_KEY_DIR}/${PACKAGER}-key.pem\"" > ${ABUILD_KEY_DIR}/abuild.conf \
